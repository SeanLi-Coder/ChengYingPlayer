...
从 **v0.2.40** 的发布流程起，为上一正式版生成较小的增量补丁。已安装版本匹配时，自动更新优先下载补丁，仍显示下载与安装进度；无需另开设置。跳过多个版本、旧 App 内容被修改、补丁损坏／无法应用，或补丁不比完整包小时，使用完整 DMG 安全回退，不保证每次升级都能走增量。手动安装仍使用完整 DMG。

更新清单、完整 DMG 和增量补丁均使用本项目独立 Ed25519 密钥签名。发行前实际应用补丁，逐文件确认与完整包中的新 App 一致；客户端验证下载及安装结果，签名错误、下载中断、磁盘不可写或安装失败不会被当成成功。只更新程序，不迁移或删除媒体、模型、下载记录和用户设置。旧版迁移也保留已保存的播放条位置、自动隐藏、剩余时间和自动检查更新开关，不用默认值覆盖你的选择。请先将 App 拖入「应用程序」并推出 DMG；直接从安装磁盘或 macOS 临时隔离位置运行时，会提示先完成正常安装。
...
后续每个正式版都必须通过自动更新发布门禁：版本号和构建号递增、与上一正式版保持相同应用身份／更新地址／签名公钥，并使用旧版公钥验证新的签名清单。草稿中的六个基础附件，以及清单列出的全部增量包和校验和，全部上传并校验后才能发布；缺失附件的临时上传失败会重试，已校验的附件会跳过，同名内容冲突则停止而不覆盖。发布后还会不带账号凭据访问正式版、播放器实际使用的更新清单，并完整下载 DMG 和所有增量包核对大小和 SHA-256，确认更新实际可取。草稿或仅构建成功不算更新已上线。用户主动关闭自动更新的选择仍会保留，不会被升级重置。
...
视频右上角的文件列表、图片窗口右侧均提供「文件夹」浏览：文件夹排在前面，双击进入任意层级子目录，点向上箭头返回上一级。列表同时显示视频、音频、图片及 Finder 颜色标签，默认按名称排序，也可按文件大小、修改日期或创建日期排序。颜色筛选只筛媒体文件，文件夹仍保留，便于继续寻找子目录中的文件。

浏览目录不会改变当前视频队列或打断动图、幻灯片；双击媒体文件才打开它。视频列表可切回「播放队列」，图片窗口可切回「图片列表」，保留原来的队列操作和显式多选顺序。只有子文件夹、没有直接媒体文件的目录也可以打开浏览。目录按层读取，不递归扫描整个磁盘；隐藏项、软件包、符号链接及 Finder 替身不会自动加入，读取失败会提示且可刷新重试。
...
视频默认按显示分辨率打开窗口（计入方向与像素宽高比），Retina 下不再自动缩成一半；超过当前显示器可用区域时等比缩小，小视频仍遵守能完整显示控件与文件列表的最小窗口尺寸。播放列表切换视频也会重新适配。已明确保存的物理像素、开窗时机、缩放倍数和初始位置偏好仍然保留，可在设置中调整。
...
- 默认输出高质量 JPG，保持视频原始分辨率；JPG 是 8 位有损格式，不等于像素无损。
- 可切换「PNG · 无损」保留位深和透明度，浮点或超高位深素材会自动使用 EXR；选择会记住。HDR、透明或广色域等不适合 JPG 的素材会提示改用无损模式，不静默丢弃这些信息。
- 播放器普通截图也默认使用高质量 `.jpg`；升级不会覆盖用户已明确保存的 PNG 等截图格式偏好。图片格式转换的默认值不受影响。
