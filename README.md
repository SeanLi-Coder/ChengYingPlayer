...
原生界面采用石墨色与冰蓝点缀，提供深浅两套外观：欢迎页采用紧凑居中布局，仅呈现打开文件、拖放与下载入口，不展示最近打开或继续上次的文件记录，也不读取历史文件名、路径和播放时间；原有播放历史、菜单与手动打开文件时的断点续播不受影响。工具面板按操作分组，并将主要导出按钮固定在底部；AI 字幕的生成、模型和任务状态以独立卡片呈现。播放器时间轴与侧栏采用统一的强调色，保留原有快捷键和无障碍操作，不添加影响观影的装饰动画。
