...
从 **v0.2.18** 起，启动时自动检查本仓库最新正式版，发现更新后直接下载，独立窗口显示已下载大小和进度条；验证、解压与安装也显示状态。没有更新或启动检查暂时离线时，不打扰使用。通过 **澄影视界 → 检查更新…** 可随时查看或重试，并可在更新窗口取消勾选「自动检查并下载更新」。
...
**v0.2.17 及以前没有配置可用更新源与公钥，需要手动安装 v0.2.18 一次；之后的新版本才支持这套自动更新。** Ed25519 更新签名不等于 Apple Developer ID 公证，首次安装的系统安全确认仍按上面的说明处理。
