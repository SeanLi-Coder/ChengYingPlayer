...
SOURCE_DIR="$PROJECT_ROOT/deps/download-center"
if [[ -z "${TARGET_BUILD_DIR:-}" || -z "${CONTENTS_FOLDER_PATH:-}" ]]; then
  echo "This script must run as an application Xcode build phase." >&2
  exit 2
fi
DESTINATION="$TARGET_BUILD_DIR/$CONTENTS_FOLDER_PATH/Helpers/DownloadCenter"
...
if [[ ! -x "$SOURCE_DIR/chengying-download-center-helper" || ! -d "$SOURCE_DIR/_internal" || ! -x "$SOURCE_DIR/deno" ]]; then
...
trap cleanup EXIT
ditto --noqtn "$SOURCE_DIR" "$STAGING_DIR/DownloadCenter"
if [[ -e "$DESTINATION" ]]; then
  mv "$DESTINATION" "$STAGING_DIR/previous"
fi
if ! mv "$STAGING_DIR/DownloadCenter" "$DESTINATION"; then
  if [[ -d "$STAGING_DIR/previous" ]]; then
    mv "$STAGING_DIR/previous" "$DESTINATION"
  fi
  exit 3
fi
codesign --verify --strict "$DESTINATION/chengying-download-center-helper"
codesign --verify --strict "$DESTINATION/deno"
codesign --verify --strict "$DESTINATION/_internal/playwright/driver/node"
