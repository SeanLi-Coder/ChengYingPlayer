...
Distribution builds use [`other/build_playback_libraries.sh`](other/build_playback_libraries.sh), not playback libraries extracted from an upstream application DMG. [`other/verify_playback_distribution.py`](other/verify_playback_distribution.py) requires matching source records, headers, configuration, license texts, library hashes and a closed dependency graph before source packaging or legal-notice installation. All corresponding-source materials and build/install scripts must accompany the binary through the same release. A developer signature or notarization does not replace these obligations.
