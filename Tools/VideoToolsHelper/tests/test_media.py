...
def test_exclusive_publish_never_overwrites_an_existing_file(tmp_path: Path) -> None:
    desired = tmp_path / "clip.mp4"
    desired.write_bytes(b"existing")
    temporary = tmp_path / ".clip.partial.mp4"
    temporary.write_bytes(b"new")

    published = ExportManager._publish(temporary, desired)

    assert published == tmp_path / "clip_2.mp4"
    assert desired.read_bytes() == b"existing"
    assert published.read_bytes() == b"new"
    assert not temporary.exists()


def test_cancellable_capture_stops_a_running_process() -> None:
...
def test_precise_export_rejects_dynamic_hdr_metadata(
    ...
    metadata["dynamic_hdr_metadata_types"] = ("HDR10+ Dynamic Metadata",)
...
def test_export_manager_validates_range_before_starting_worker(
    sample_video: Path,
    tmp_path: Path,
    ffmpeg: str,
    ffprobe: str,
    ...
    with pytest.raises(MediaError, match="later"):
        manager.create(source, start=4, end=2, output_directory=tmp_path)

    assert manager._jobs == {}


def test_export_worker_start_failure_does_not_leave_a_stuck_job(
    ...
    monkeypatch.setattr(media_module.threading, "Thread", FailingThread)

    with pytest.raises(MediaError, match="Could not start export worker"):
        manager.create(source, start=1, end=2, output_directory=tmp_path)
...
def test_cancelled_frame_extraction_removes_partial_directory(
    ...
    monkeypatch.setattr("media.subprocess.Popen", SlowProcess)
