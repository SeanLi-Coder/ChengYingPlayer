...
def _read_pixel_format_catalog(
    ffprobe: str,
    *,
    cancel_event: threading.Event | None = None,
...
@lru_cache(maxsize=8)
def _cached_pixel_format_catalog(ffprobe: str) -> dict[str, dict[str, Any]]:
    return _read_pixel_format_catalog(ffprobe)


def _pixel_format_catalog(
    ffprobe: str,
    *,
    cancel_event: threading.Event | None = None,
) -> dict[str, dict[str, Any]]:
    if cancel_event is None:
        return _cached_pixel_format_catalog(ffprobe)
    return _read_pixel_format_catalog(ffprobe, cancel_event=cancel_event)
...
def probe_video(
    ...
    *,
    ...
    pixel_details = _pixel_format_catalog(
        ffprobe_path,
        cancel_event=cancel_event,
    ).get(pixel_format, {})
...
def _remove_owned_file(path: Path | None) -> str | None:
    if path is None or not os.path.lexists(path):
        return None
    try:
        path.unlink()
    except OSError as exc:
        return f"Could not remove partial output {path.name}: {exc}"
    if os.path.lexists(path):
        return f"Could not remove partial output {path.name}"
    return None
...
class ExportManager:
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            raise MediaError("Could not start export worker") from exc
        return job
    ...
    def _ffv1_pixel_format(self, source: VideoSource) -> str:
        ...
        for candidate in candidates:
            candidate_details = _pixel_format_catalog(
                self.ffprobe,
                cancel_event=self.cancel_event,
            ).get(candidate, {})
    ...
    def _publish(temporary_path: Path, desired_path: Path) -> Path:
        for number in range(1, 100_000):
            ...
            try:
                _exclusive_rename(temporary_path, destination)
    ...
    def _run(self, job: ExportJob) -> None:
        ...
        try:
            ...
            if (
                ...
                and result_metadata.get("static_hdr_metadata") != source_hdr_metadata
            ):
            ...
            with job.lock:
                ...
                job.progress = 100.0
                job.message = "Clip completed; the source video was not modified"
                job.finished_at = time.time()
        except InterruptedError:
            cleanup_error = _remove_owned_file(temporary_path)
            with job.lock:
                job.status = "failed" if cleanup_error else "cancelled"
                job.error = cleanup_error
                job.message = (
                    "Export cancellation could not clean partial output"
                    if cleanup_error
                    else "Export cancelled"
                )
                job.finished_at = time.time()
        except Exception as exc:  # noqa: BLE001 - worker boundaries must record failures
            cleanup_error = _remove_owned_file(temporary_path)
            with job.lock:
                job.status = "failed"
                job.error = str(exc) or exc.__class__.__name__
                if cleanup_error:
                    job.error = f"{job.error}; {cleanup_error}"
                job.message = "Export failed; the source video was not modified"
                job.finished_at = time.time()
        finally:
            cleanup_error = _remove_owned_file(temporary_path)
            with job.lock:
                if cleanup_error:
                    job.status = "failed"
                    job.error = (
                        f"{job.error}; {cleanup_error}" if job.error else cleanup_error
                    )
                    job.message = "Export failed while cleaning partial output"
                    job.finished_at = time.time()
                process = job.process
                job.process = None
            if process is not None and process.poll() is None:
...
class RotationManager(ExportManager):
    ...
    def create(self, source: VideoSource, *, degrees: int) -> RotationJob:
        ...
        try:
            ...
            with self._lock:
                self._jobs.pop(job.id, None)
            raise MediaError("Could not start rotation worker") from exc
        return job
    ...
    def cancel(self, job_id: str) -> RotationJob:
        ...
        with job.lock:
            ...
            job.cancel_event.set()
            process = job.process
            job.message = "Cancelling and removing the partial video"
        if process is not None and process.poll() is None:
            self._request_process_stop(process)
            threading.Thread(
                target=self._escalate_process_stop,
        ...
        try:
            ...
            with job.lock:
                if job.cancel_event.is_set():
                    raise InterruptedError
                job.status = "running"
                job.message = "Inspecting HDR and display metadata frame by frame"
                job.started_at = time.time()
            self._scan_all_frame_hdr_metadata(job)
            if job.cancel_event.is_set():
                raise InterruptedError
            with job.lock:
                job.message = f"Permanently rotating frames {job.degrees}° clockwise"
            options: dict[str, Any] = {
                "stdout": subprocess.PIPE,
                "stderr": subprocess.PIPE,
                "text": True,
            ...
            with job.lock:
                job.message = "Verifying rotation, video quality, and audio"
            ...
            with job.lock:
                ...
                job.status = "completed"
                job.progress = 100.0
                job.message = f"Permanent {job.degrees}° clockwise rotation completed"
                job.finished_at = time.time()
            published_path = None
        except InterruptedError:
            cleanup_errors = [
            ...
            with job.lock:
                job.status = "failed" if cleanup_errors else "cancelled"
                job.error = "; ".join(cleanup_errors) or None
                job.message = (
                    "Rotation cancellation could not clean partial output"
                    if cleanup_errors
                    else "Rotation cancelled and partial video removed"
                )
                job.finished_at = time.time()
        except Exception as exc:  # noqa: BLE001 - worker boundaries must record failures
            cleanup_errors = [
            ...
            with job.lock:
                ...
                job.error = str(exc) or exc.__class__.__name__
                if cleanup_errors:
                    job.error = f"{job.error}; {'; '.join(cleanup_errors)}"
                job.message = "Rotation failed; the source video was not modified"
                job.finished_at = time.time()
        finally:
            cleanup_error = _remove_owned_file(temporary_path)
            with job.lock:
                if cleanup_error:
                    ...
                    job.message = "Rotation failed while cleaning partial output"
                    job.finished_at = time.time()
                process = job.process
                job.process = None
            if process is not None and process.poll() is None:
...
class FrameExtractionJob:
    ...
    progress: float = 0.0
    frame_count: int = 0
    message: str = "Waiting to extract frames"
    error: str | None = None
    created_at: float = field(default_factory=time.time)
    started_at: float | None = None
    finished_at: float | None = None
...
class FrameExtractionManager:
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            with self._lock:
                self._jobs.pop(job.id, None)
            raise MediaError("Could not start frame extraction worker") from exc
        return job
    ...
    def cancel(self, job_id: str) -> FrameExtractionJob:
        ...
        with job.lock:
            ...
            job.cancel_event.set()
            process = job.process
            job.message = "Cancelling and removing partial frame images"
        if process is not None and process.poll() is None:
    ...
    def _verify_frames(self, job: FrameExtractionJob, frames: list[Path]) -> None:
        ...
        if job.frame_extension == ".png":
            ...
            for path in frames:
                if job.cancel_event.is_set():
                    raise InterruptedError
                stream = self._probe_still(path, cancel_event=job.cancel_event)
                dimensions = (
    ...
    def _remove_owned_directory(directory: Path | None) -> str | None:
        ...
        if directory.is_symlink() or not directory.is_dir():
            return _remove_owned_file(directory)
        try:
            ...
            return f"Could not remove partial frame directory {directory.name}: {exc}"
        if os.path.lexists(directory):
            return f"Could not remove partial frame directory {directory.name}"
        return None
    ...
    def _publish_directory(self, temporary_directory: Path, desired_path: Path) -> Path:
        with self._publish_lock:
            for number in range(1, 100_000):
                ...
                try:
                    _exclusive_rename(temporary_directory, destination)
                except FileExistsError:
                    continue
                except OSError as exc:
    ...
    def _run(self, job: FrameExtractionJob) -> None:
        ...
        try:
            ...
            with job.lock:
                job.message = "Verifying every extracted frame"
            ...
            with job.lock:
                ...
                job.error = "; ".join(cleanup_errors) or None
                job.message = (
                    "Frame cancellation could not clean partial images"
                    if cleanup_errors
                    else "Frame extraction cancelled and partial images removed"
                )
                job.finished_at = time.time()
        except Exception as exc:  # noqa: BLE001 - worker boundaries must record failures
            cleanup_errors = [
            ...
            with job.lock:
                ...
                if cleanup_errors:
                    job.error = f"{job.error}; {'; '.join(cleanup_errors)}"
                job.message = "Frame extraction failed and partial images were removed"
                job.finished_at = time.time()
        finally:
            cleanup_errors = [
                error
            ...
            with job.lock:
                if cleanup_errors:
                    ...
                    job.message = "Frame extraction failed while cleaning partial images"
                    job.finished_at = time.time()
                process = job.process
                job.process = None
            if process is not None and process.poll() is None:
